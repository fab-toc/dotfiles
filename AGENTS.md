...
- **State goes in `$XDG_STATE_HOME`, not the repository.** Machine state, shell history, caches. A file inside the repository is at risk from `git clean -xdf` and breaks invariant 1.
- **Respect the XDG Base Directory specification** wherever a tool supports it. Where a tool does not — `~/.ssh` — leave it alone.
- **Never add key material or secrets.** This repository is public. This extends past keys to anything that describes real infrastructure: hostnames, internal addresses, ports. Those go in a local config — see `modules/openssh/.ssh/config` for the pattern.
- **The installer is POSIX `sh`**, not bash: `install.sh` is its own entry point, fetched and piped to `sh`, and Debian's `/bin/sh` is dash. Assume only `sh`, `git`, `curl`, and a package manager — `jq` is installed by the installer before it reads the manifest, and is the only exception. Lint with `shellcheck -s sh`, format with `shfmt -s`.
- **The repository's location is free but fixed once chosen.** Nothing may hardcode `~/.dotfiles`; the installer takes its own directory and records it in machine state. Stow encodes that path into every symlink, so a move is a deliberate unstow-and-reinstall, never a silent adoption.
...
- `install.sh` — the installer, and its own piped entry point: with no checkout around it, it clones one and re-execs itself from there.
